...
class JWTAuthenticationFilterTest {
    ...
    private static final String INVALID_JWT = "SOME_INVALID_JWT";

    private static final String BEARER = "Bearer ";
    private static final String AUTH_HEADER_VALUE = BEARER + EXAMPLE_JWT;
    private static final String INVALID_AUTH_HEADER_VALUE = BEARER + INVALID_JWT;
    private static final String INVALID_AUTH_HEADER_FORMAT_VALUE = "ABCD " + EXAMPLE_JWT;
    ...
    @Test
    void doFilterInternalWithInvalidJWTHeaderValueNoAuthProcessing() throws ServletException, IOException {
        when(mockHttpServletRequest.getHeader(AUTHORIZATION_HEADER_NAME)).thenReturn(INVALID_AUTH_HEADER_VALUE);
        when(authTokenService.validateAndGetUserId(INVALID_JWT)).thenReturn(Optional.empty());
    ...
    @Test
    void doFilterInternalWithUserNotFoundNoAuthProcessing() throws ServletException, IOException {
        when(mockHttpServletRequest.getHeader(AUTHORIZATION_HEADER_NAME)).thenReturn(AUTH_HEADER_VALUE);
        when(authTokenService.validateAndGetUserId(EXAMPLE_JWT)).thenReturn(Optional.of(EXAMPLE_USER_ID));
        when(authUserDetailsService.loadUserByUsername(EXAMPLE_USER_ID)).thenReturn(null);
    ...
    @Test
    void doFilterInternalWithInvalidJWTHeaderFormatNoAuthProcessing() throws ServletException, IOException {
        when(mockHttpServletRequest.getHeader(AUTHORIZATION_HEADER_NAME)).thenReturn(INVALID_AUTH_HEADER_FORMAT_VALUE);
